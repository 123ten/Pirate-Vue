...
export default class TableSettings<RecordType = DefaultRecordType, QueryForm = DefaultRecordType, Fields extends DefaultFieldsType = DefaultFieldsType> implements TableSettingsType<RecordType, QueryForm, Fields> {
  ...
  /**
   * 给formRefs赋值
   * @param isEditting {boolean} 是否编辑
   */
  private initFormRefs = (isEditting?: boolean) => {
    this.formRefs = Form.useForm(this.form.fields, this.transformRules(this.form.rules, isEditting));
  }
  ...
  public queryAll = async () => {
    ...
    try {
      const {data} = await this.api?.request(this.customParams?.('queryAll', params) || params);
  ...
  public deleteByIds = async (type: Extract<Operation, 'delete' | 'row-delete'>, ids: Key[]) => {
  ...
  public confirmForm = async () => {
    ...
    this.form.loading = true
    try {
      await this.api.upsertRequest(this.customParams?.('confirmForm', fields) || fields)
      notification.success({
        message: t("message.success"),
   ...
   * 转换rules校验
   * @param rules 校验规则
   * @param isEditting {boolean} 是否编辑 默认 非编辑
   */
  private transformRules = (rules?: Rules | ((isEditting: boolean, fields: Fields) => Rules), isEditting: boolean = false) => {
    if (typeof rules === 'function') {
      return rules(isEditting, this.form.fields as Fields)
    }
    return rules
  }
}
